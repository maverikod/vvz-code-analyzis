...
def tokens(s):
    """Return a rough token count for a string."""
    return len(s) // 4
...
"""
print(
    f"CST modify_tree payload:  {tokens(cst_modify):>5} tokens  ({len(cst_modify)} chars)"
)
...
benefits = [
    ...
    (
        ...
        "DB index stays in sync (ast_trees, cst_trees, entities)",
    ),
]
