...
3. ⚠️ Вынести все хардкоды из кода в константы - **частично выполнено, найдены хардкоды**:
   - `code_analysis/commands/code_mapper_mcp_command.py`: строка 531 (`max_lines=400`), строка 648 (игнорируемые директории)
   - `code_analysis/core/comprehensive_analyzer.py`: строка 37 (`max_lines=400`), строки 53-64 (паттерны плейсхолдеров)
   - `code_analysis/commands/database_restore_mcp_commands.py`: строка 83 (игнорируемые директории)
   - `code_analysis/commands/worker_management_mcp_commands.py`: строка 240 (игнорируемые паттерны)
   - `code_analysis/main.py`: строки 335, 456 (`"data" / "code_analysis.db"`), строки 338, 458 (`"./logs"`), строки 883-888 (числовые константы)
   - `code_analysis/commands/base_mcp_command.py`: строка 273 (`max_lines=400`)
...
8. ⚠️ Обновить все модули для использования `get_settings()` вместо прямого доступа к константам - **частично выполнено, есть хардкоды** (см. пункт 3)
...
- [x] Все хардкоды вынесены в файл констант - **✅ выполнено** (все найденные хардкоды заменены на константы)
- [x] Создан SettingsManager с приоритетом CLI > ENV > Constants
- [x] Все модули обновлены для использования констант - **✅ выполнено** (все хардкоды заменены на константы из constants.py)
- [ ] Обновить config.py для использования SettingsManager (не критично, см. Этап 3.5, пункт 6)
...
- [x] Заменить хардкоды на константы в найденных файлах (см. Этап 3.5, пункт 3) - **✅ выполнено**
...
**Числовые константы** (найдены хардкоды, требуют замены):
- `400` - максимальное количество строк в файле
  - ❌ `code_analysis/core/comprehensive_analyzer.py:37` - `max_lines: int = 400`
  - ❌ `code_analysis/commands/code_mapper_mcp_command.py:531` - `max_lines: int = 400`
  - ❌ `code_analysis/commands/base_mcp_command.py:273` - `max_lines=400`
  - ✅ Константа: `DEFAULT_MAX_FILE_LINES = 400`
- `30` - минимальная длина чанка, интервал опроса (секунды)
  - ❌ `code_analysis/main.py:884,888` - `poll_interval = 30`
...
- `10` - размер батча, задержка повтора (секунды), таймаут git команд (секунды)
  - ❌ `code_analysis/main.py:883,887` - `batch_size = 10`
...
**Пути и имена файлов** (найдены хардкоды, требуют замены):
- Все пути к директориям и файлам должны быть вынесены в константы
  - ❌ `code_analysis/main.py:335,456` - `"data" / "code_analysis.db"` → `DEFAULT_DB_PATH`
  - ❌ `code_analysis/main.py:338,458` - `"./logs"` → `LOGS_DIR_NAME`
...
**Места использования хардкодов** (найдены, требуют замены на константы):
...
- `code_analysis/core/comprehensive_analyzer.py`: 
  - строка 37: `max_lines: int = 400` → `DEFAULT_MAX_FILE_LINES`
  - строки 53-64: хардкод паттернов плейсхолдеров → `PLACEHOLDER_PATTERNS`
- `code_analysis/commands/code_mapper_mcp_command.py`:
  - строка 531: `max_lines: int = 400` → `DEFAULT_MAX_FILE_LINES`
  - строка 648: `["__pycache__", "node_modules", ".git", "data", "logs"]` → использовать `DEFAULT_IGNORE_PATTERNS`, `DATA_DIR_NAME`, `LOGS_DIR_NAME`
- `code_analysis/commands/database_restore_mcp_commands.py`:
  - строка 83: `["__pycache__", "node_modules", ".git", "data", "logs"]` → использовать константы
- `code_analysis/commands/worker_management_mcp_commands.py`:
  - строка 240: `ignore_patterns=[".git", "__pycache__", "data", "logs"]` → использовать константы
- `code_analysis/main.py`:
  - строки 335, 456: `"data" / "code_analysis.db"` → `DEFAULT_DB_PATH`
  - строки 338, 458: `"./logs"` → `LOGS_DIR_NAME`
  - строки 883-888: `batch_size = 10`, `poll_interval = 30` → `DEFAULT_BATCH_SIZE`, `DEFAULT_POLL_INTERVAL`
- `code_analysis/commands/base_mcp_command.py`:
  - строка 273: `max_lines=400` → `DEFAULT_MAX_FILE_LINES`
- `code_analysis/core/storage_paths.py`: пути к директориям (требует проверки)
- `code_analysis/core/vectorization_worker_pkg/runner.py`: значения по умолчанию для параметров (требует проверки)
