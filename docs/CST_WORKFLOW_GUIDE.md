...
| Replace function or class body / single block | `cst_modify_tree` replace (after `cst_load_file` + `cst_find_node` or `cst_get_node_by_range`) | `compose_cst_module` with selector (e.g. `kind: "function"`, `kind: "method"`) |
| Insert line(s) at module level | `cst_modify_tree` insert with `parent_node_id: "__root__"` | `compose_cst_module` with appropriate selector if it fits |
| Insert line(s) inside function/class | `cst_modify_tree` insert with `parent_node_id` = that FunctionDef/ClassDef node_id | `compose_cst_module` with selector for the block |
| Single patch by selector (one replace) | `compose_cst_module` with one op | `query_cst` with `replace_with`/`code_lines`, or tree-based flow |
| Multiple related edits in one file | `cst_load_file` → `cst_modify_tree` (several ops) → `cst_save_tree` | Multiple `compose_cst_module` calls (one file write per call) |
