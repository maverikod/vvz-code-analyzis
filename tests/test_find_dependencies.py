...
class TestFindDependencies:
    ...
    async def test_find_dependencies_in_usages(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            db.add_usage(file1_id, 15, "attribute_access", "property", "test_prop", "TestClass", "context3")
            
            # Find dependencies for test_method
            cmd = FindDependenciesCommand(db, project_id, "test_method", entity_type="method")
            result = await cmd.execute()
    ...
    async def test_find_dependencies_in_imports(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            cmd = FindDependenciesCommand(db, project_id, "pathlib", entity_type="module")
            result = await cmd.execute()
    ...
    async def test_find_dependencies_with_target_class(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            cmd = FindDependenciesCommand(db, project_id, "test_method", entity_type="method", target_class="Class1")
            result = await cmd.execute()
