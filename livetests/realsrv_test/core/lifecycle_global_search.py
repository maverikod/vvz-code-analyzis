...
async def _seed_throwaway_project(
    ...
    *,
    ...
    only when ``create_project`` itself never returned one; once a project
    exists, ``project_id`` is returned even if a later seed step fails (a
    broken seed step must not raise, but it also must not lose the id the
    caller needs to clean the half-seeded project up — bug d5835fbf). In
    that case ``reason`` carries the verbatim failing step's outcome/
    exception so the caller never reports an opaque "seed step failed".

    Args:
        client: Connected async client.
        watch_dir_id: Watch directory to create the throwaway project under.
        token: Shared marker token seeded into the fixture file.
        label: ``"a"`` or ``"b"`` — distinguishes the two throwaway projects
            in step/error text.
        project_name: Name to create the project under — generated by the
            caller (not here) so it stays known to the caller for teardown
            regardless of where seeding fails.
    """
    outcome, data = await call_step_with_data(
        client,
        "create_project",
    ...
    if session_outcome.status is not Status.EXECUTED_OK:
        return project_id, None, f"session_create: {session_outcome.reason}"
    ...
    try:
        ...
        return project_id, None, f"file_sessions.upload_new: {exc!r}"
    if not file_id:
        return project_id, None, "file_sessions.upload_new returned no file_id"
    ...
    if index_outcome.status is not Status.EXECUTED_OK:
        return project_id, None, f"update_indexes: {index_outcome.reason}"
...
async def run_global_search_attribution_check(
    ...
    # Every throwaway project this check creates (id, name), appended as
    # soon as create_project returns an id -- even a half-seeded project
    # (later step failed) is tracked, so the finally below cleans it up
    # too. Before this fix, this check had NO cleanup at all: 109 leaked
    # "verify-global-search-*" projects observed (bug d5835fbf).
    created: List[Tuple[str, str]] = []
    try:
        project_a_name = f"verify-global-search-a-{uuid.uuid4().hex[:8]}"
        ...
        if project_a:
            created.append((project_a, project_a_name))
        if not project_a:
            return _outcome(
                Status.FAILED, f"seed step failed for throwaway project A: {reason_a}"
            )

        project_b_name = f"verify-global-search-b-{uuid.uuid4().hex[:8]}"
        ...
        if project_b:
            created.append((project_b, project_b_name))
        if not project_b:
            return _outcome(
        ...
        if (
            grep_outcome.status is not Status.EXPECTED_ERROR
            and grep_outcome.status is not Status.FAILED
        ):
            return _outcome(
                Status.FAILED,
        ...
        for project_id, project_name in created:
            await purge_disposable_project(client, project_id, project_name)
