...
class BackupManager:
    ...
    # NOTE: create_database_backup / _is_database_empty (SQLite .db file + .wal/.shm/.journal
    # sidecar backup, via the stdlib sqlite3 module) were removed with SQLite support.
    # PostgreSQL backups go through core.postgres_cli_backup (pg_dump -Fc), not BackupManager.
    # File-backup functionality above (create_backup, list_versions, restore_file, etc.) is
    # unrelated to the database file and remains intact for file_handlers pre-write backups.
