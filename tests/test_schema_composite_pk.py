...
from code_analysis.core.database_driver_pkg.drivers.sqlite_tables import run_create_table


_COMPOSITE_SCHEMA = {
    "name": "watch_dir_paths",
    "columns": [
        {"name": "server_instance_id", "type": "TEXT", "not_null": True, "primary_key": True},
...
def test_sqlite_composite_pk_creates_table() -> None:
    ...
    pk_cols = [
        row[1] for row in conn.execute("PRAGMA table_info(watch_dir_paths)") if row[5] > 0
    ]
...
def test_sqlite_full_schema_definition_creates_all_tables() -> None:
    ...
    items = tables.items() if isinstance(tables, dict) else [
        (t.get("name"), t) for t in tables
    ]
