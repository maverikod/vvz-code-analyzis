...
def real_test_file_in_db(
    ...
    file_id = test_db.add_file(
        ...
        or file_content.strip().startswith("'''"),
    ...
    def test_file_splitter_updates_database_real(
        ...
        if splitter.tree:
            for node in splitter.tree.body:
                if isinstance(node, type(splitter.tree.body[0])) and hasattr(
                    node, "name"
                ):
...
class TestClassSplitterIntegrationReal:
    ...
    def test_class_splitter_updates_database_real(
        ...
        if success:
            ...
            assert (
                f"{source_class}1" in content or f"{source_class}2" in content
            ), "New classes should be in file"
