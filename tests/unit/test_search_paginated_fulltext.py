...
@pytest.mark.asyncio
async def test_run_paginated_fulltext_publishes_block_and_returns_1(tmp_path: Path) -> None:
    session, layout = _session_and_layout(tmp_path)

    command = MagicMock()
    command.execute = AsyncMock(
        return_value=SuccessResult(data={"results": [{"file_path": "a.py", "chunk_text": "x"}], "count": 1})
    )
