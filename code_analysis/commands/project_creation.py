...
class CreateProjectCommand:
    ...
    def _get_watch_dir_path(self) -> Optional[Path]:
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                rows = result
            if not rows:
                logger.error(f"Watch directory {self.watch_dir_id} not found in database")
                return None
            path_str = rows[0].get("absolute_path") if isinstance(rows[0], dict) else rows[0]
            if not path_str:
            ...
            logger.error(f"Error getting watch_dir path for {self.watch_dir_id}: {e}", exc_info=True)
            return None
    ...
    def _scaffold_dirs(self, project_path: Path) -> List[str]:
        """
            ...
            """
        dirs = ["tests", "docs/plans", "docs/aireports", "docs/agents", "logs", "configs", "scripts"]
        created: List[str] = []
        for rel in dirs:
            d = project_path / rel
    ...
    def _create_venv_sync(self, project_path: Path) -> Tuple[bool, Optional[str]]:
        ...
        if venv_path.exists():
            ...
            return True, None
        try:
            r = subprocess.run([sys.executable, "-m", "venv", str(venv_path)], capture_output=True, text=True, timeout=120)
            if r.returncode != 0:
    ...
    async def _run_bootstrap(self, project_path: Path, slug: str) -> Dict[str, Any]:
        ...
        if self.scaffold:
            try:
                report["scaffold_dirs"] = {"ok": True, "created": self._scaffold_dirs(project_path)}
            except Exception as e:
            ...
            if self.template_zip:
                ...
                if err:
                    report["template"] = {"ok": False, "error": err, "extracted": extracted}
                else:
                    report["template"] = {"ok": True, "extracted": extracted}
            else:
                report["template"] = {"ok": True, "skipped": "no template_zip provided"}
            try:
                report["base_files"] = {"ok": True, "created": self._create_base_files(project_path, slug)}
            except Exception as e:
        ...
        if self.create_venv:
            ok, err = await asyncio.get_event_loop().run_in_executor(None, self._create_venv_sync, project_path)
            report["venv"] = {"ok": ok, "path": str(project_path / ".venv")} if ok else {"ok": False, "error": err}
        else:
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if not watch_dir_path:
                return {"success": False, "error": "WATCH_DIR_NOT_FOUND", "message": f"Watch directory {self.watch_dir_id} not found in database or path not set"}
            if not self.project_name or not self.project_name.strip():
                return {"success": False, "error": "INVALID_PROJECT_NAME", "message": "Project name cannot be empty"}
            slug = self.project_name.strip()
            project_path = watch_dir_path / slug
            if project_path.exists():
                from .base_mcp_command import BaseMCPCommand
                existing_project_id = BaseMCPCommand._get_project_id_by_root_path(self.database, str(project_path))
                if existing_project_id:
                    return {"success": False, "error": "PROJECT_ALREADY_EXISTS", "message": f"Project directory already exists and is registered: {existing_project_id}", "existing_project_id": existing_project_id}
                if not self.use_existing_dir:
                    return {"success": False, "error": "PROJECT_DIR_EXISTS", "message": f"Project directory already exists: {project_path}"}
                project_id = self.project_id or str(uuid.uuid4())
                projectid_path = project_path / "projectid"
                projectid_path.write_text(json.dumps({"id": project_id, "description": self.description}, indent=4, ensure_ascii=False) + "\n", encoding="utf-8")
                logger.info(f"Created projectid file in existing directory: {projectid_path}")
                self.database.execute("INSERT INTO projects (id, root_path, name, comment, watch_dir_id, updated_at) VALUES (?, ?, ?, ?, ?, julianday('now'))", (project_id, str(project_path), project_path.name, self.description, self.watch_dir_id))
                logger.info(f"Registered project in database: {project_id}")
                bootstrap_report = await self._run_bootstrap(project_path, slug)
                return {"success": True, "project_id": project_id, "message": f"Created projectid in existing directory and registered: {project_id}", "bootstrap_report": bootstrap_report}
            project_id = self.project_id or str(uuid.uuid4())
            ...
            try:
                ...
                self.database.execute("INSERT INTO projects (id, root_path, name, comment, watch_dir_id, updated_at) VALUES (?, ?, ?, ?, ?, julianday('now'))", (project_id, str(project_path), project_path.name, self.description, self.watch_dir_id), transaction_id=transaction_id)
                logger.info(f"Registered project in database: {project_id}")
                ...
                raise
            bootstrap_report = await self._run_bootstrap(project_path, slug)
            return {"success": True, "project_id": project_id, "message": f"Created and registered new project: {project_id}", "bootstrap_report": bootstrap_report}
        except Exception as e:
            logger.error(f"Error creating project: {e}", exc_info=True)
            return {"success": False, "error": "CREATE_PROJECT_ERROR", "message": f"Failed to create project: {str(e)}"}
