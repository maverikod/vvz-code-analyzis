...
| `.cursor/agents/` (repo root) | Cursor subagent definitions (`orchestrator_debug` / `orchestrator_tactical_debug` for informal work). Includes **`tester_ca`** — MCP-only programmer–tester for **`test_data/`** and server-watched projects. |
