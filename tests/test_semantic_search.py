...
def test_db():
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        method_id = db.add_method(
            class_id, "test_method", 15, ["self"], "Test method docstring", False, False, False
        )
...
class TestSemanticSearchBasic:
    ...
    @pytest.mark.asyncio
    async def test_search_basic(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_result_structure(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_with_limit(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_with_max_distance(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_empty_results(self, test_db, mock_faiss_manager, mock_svo_client_manager):
        """Test search when FAISS returns no results."""
        db_path, project_id, tmpdir = test_db
        database = CodeDatabase(db_path)
        
        # Mock FAISS to return empty results
        mock_faiss_manager.search = Mock(return_value=(
    ...
    @pytest.mark.asyncio
    async def test_search_with_ast_node(self, test_db, mock_faiss_manager, mock_svo_client_manager):
        ...
        if file_id:
            ...
            await database.save_ast_tree(file_id, project_id, ast_json, "test-hash", 1234567890.0)
        ...
        try:
            ...
            has_ast_node = any("ast_node" in result for result in results)
    ...
    @pytest.mark.asyncio
    async def test_search_context_binding(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_embedding_without_embedding_attr(self, test_db, mock_faiss_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_ast_node_name_priority(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_no_ast_node_name(self, test_db, mock_faiss_manager, mock_svo_client_manager):
        ...
        try:
            ...
            for result in results:
                if result.get("chunk_text") == "Standalone docstring":
                    ...
                    assert "ast_node_name" not in result or result.get("ast_node_name") is None
                    break
    ...
    @pytest.mark.asyncio
    async def test_search_ast_node_json_parsing_error(self, test_db, mock_faiss_manager, mock_svo_client_manager):
        ...
        file_id = database.get_file_id("test.py", project_id)
        if file_id:
            await database.save_ast_tree(file_id, project_id, "invalid json", "test-hash", 1234567890.0)
    ...
    @pytest.mark.asyncio
    async def test_search_ast_node_not_found(self, test_db, mock_faiss_manager, mock_svo_client_manager):
...
class TestSemanticSearchEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_search_empty_query(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_search_different_project(self, test_db, mock_faiss_manager, mock_svo_client_manager):
        """Test search when vector_ids belong to different project."""
        db_path, project_id, tmpdir = test_db
        database = CodeDatabase(db_path)
        
        # Create different project
        other_project_id = database.get_or_create_project(str(tmpdir), name="other_project")
    ...
    @pytest.mark.asyncio
    async def test_search_missing_chunks(self, test_db, mock_faiss_manager, mock_svo_client_manager):
...
class TestSemanticSearchAPI:
    ...
    @pytest.mark.asyncio
    async def test_api_semantic_search(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_api_semantic_search_with_params(self, test_db, mock_faiss_manager, mock_svo_client_manager):
    ...
    @pytest.mark.asyncio
    async def test_api_init_without_db_path(self, mock_faiss_manager, mock_svo_client_manager):
