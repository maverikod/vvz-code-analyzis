...
class TestGetClassHierarchy:
    ...
    async def test_get_hierarchy_with_inheritance(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            file_id = db.add_file("test.py", 100, 1234567890.0, True, project_id)
            base_id = db.add_class(file_id, "BaseClass", 10, "Base class", [])
            derived_id = db.add_class(file_id, "DerivedClass", 20, "Derived class", ["BaseClass"])
    ...
    async def test_get_hierarchy_multiple_inheritance(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            # Add file and classes
            file_id = db.add_file("test.py", 100, 1234567890.0, True, project_id)
            base1_id = db.add_class(file_id, "Base1", 10, "Base 1", [])
            base2_id = db.add_class(file_id, "Base2", 15, "Base 2", [])
            derived_id = db.add_class(file_id, "Derived", 20, "Derived", ["Base1", "Base2"])
    ...
    async def test_get_all_hierarchies(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            base_id = db.add_class(file_id, "BaseClass", 10, "Base class", [])
            derived_id = db.add_class(file_id, "DerivedClass", 20, "Derived class", ["BaseClass"])
            standalone_id = db.add_class(file_id, "StandaloneClass", 30, "Standalone", [])
            ...
            assert len(result["hierarchies"]) >= 2  # At least BaseClass and StandaloneClass
            
            # Check that hierarchies are present
            hierarchy_names = {h["name"] for h in result["hierarchies"]}
            assert "BaseClass" in hierarchy_names or "StandaloneClass" in hierarchy_names
