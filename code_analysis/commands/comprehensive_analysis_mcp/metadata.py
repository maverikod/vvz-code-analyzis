...
def get_metadata(cls: Type[Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "detailed_description": (
            ...
            "8b. Project integrity (optional): missing files on disk, circular imports "
            "→ ``issues`` table (when file watcher idle)\n"
            ...
            "- Missing files on disk: Indexed ``files`` rows absent on disk → ``issues`` "
            "(``missing_file_on_disk``); skipped while file watcher lease active\n"
            "- Circular imports: SQL edge/tree/filter on ``imports`` → ``issues`` "
            "(``circular_import``); list via ``list_errors_by_category``\n\n"
            "Use cases:\n"
