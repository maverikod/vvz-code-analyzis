...
3. **Chain** — you work **only** through **`orchestrator_tactical_debug`**. That tactical layer administers **`coder_auto`**, **`tester_ca`**, **`researcher_code`** / **`researcher_doc`**, **`tester_auto`**, **`doc_writer`** (and **not** `planner_auto` in the default debug chain). You do **not** call those specialists directly.
...
- You do **not** call **`planner_auto`**, **`coder_auto`**, **`tester_auto`**, or **`tester_ca`** directly.
