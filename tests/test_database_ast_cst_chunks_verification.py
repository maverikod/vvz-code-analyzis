...
def test_file_with_content(test_db, test_project, tmp_path):
    ...
    ), f"update_file_data should succeed: {result.get('error')}"
...
class TestASTCSTChunksVerification:
    ...
    def test_ast_created_after_update_file_data(
        ...
        assert (
            ast_record is not None
        ), "AST tree should be created after update_file_data"
        assert (
            ast_record["file_id"] == file_id
        ), "AST tree should be linked to correct file_id"
        assert (
            ast_record["project_id"] == project_id
        ), "AST tree should be linked to correct project_id"
    ...
    def test_cst_created_after_update_file_data(
        ...
        assert (
            cst_record is not None
        ), "CST tree should be created after update_file_data"
        assert (
            cst_record["file_id"] == file_id
        ), "CST tree should be linked to correct file_id"
        assert (
            cst_record["project_id"] == project_id
        ), "CST tree should be linked to correct project_id"
        ...
        assert (
            cst_code["cst_code"] == file_content
        ), "CST code should match file content"
    ...
    def test_entities_created_after_update_file_data(
        ...
        assert (
            "standalone_function" in function_names
        ), "standalone_function should be created"
    ...
    def test_file_marked_for_chunking_after_update_file_data(
        ...
        chunks = test_db._fetchall(
            "SELECT id FROM code_chunks WHERE file_id = ?", (file_id,)
        )
        assert (
            len(chunks) == 0
        ), "Chunks should be deleted by mark_file_needs_chunking, file is ready for re-chunking"
        ...
        assert (
            file_id in file_ids_needing_chunking
    ...
    def test_ast_cst_updated_after_file_modification(
        ...
        initial_ast = test_db._fetchone(
            "SELECT ast_hash FROM ast_trees WHERE file_id = ?", (file_id,)
        )
        ...
        initial_cst = test_db._fetchone(
            "SELECT cst_hash FROM cst_trees WHERE file_id = ?", (file_id,)
        )
...
'''
        ...
        assert (
            updated_ast["ast_hash"] != initial_ast_hash
        ), "AST hash should change after modification"
        ...
        assert (
            updated_cst["cst_hash"] != initial_cst_hash
        ), "CST hash should change after modification"
        ...
        assert (
            cst_code["cst_code"] == new_content
        ), "CST code should match new file content"
        ...
        assert (
            "standalone_function" not in function_names
    ...
    def test_chunks_structure_after_creation(
        ...
        assert (
            chunk["embedding_model"] is None
        ), "embedding_model should be NULL until vectorized"
        assert (
            chunk["embedding_vector"] is None
        ), "embedding_vector should be NULL until vectorized"
        ...
        ), "vector_id should be set after vectorization"
        assert (
            updated_chunk["embedding_model"] == "test-model"
        ), "embedding_model should be set after vectorization"
        assert (
            updated_chunk["embedding_vector"] == embedding_vector_json
        ), "embedding_vector should be set after vectorization"
    ...
    def test_full_cycle_add_and_modify_file(self, test_db, test_project, tmp_path):
...
'''
        ...
        ast_record = test_db._fetchone(
            "SELECT id, ast_hash FROM ast_trees WHERE file_id = ?", (file_id,)
        )
        ...
        cst_record = test_db._fetchone(
            "SELECT id, cst_hash, cst_code FROM cst_trees WHERE file_id = ?", (file_id,)
        )
        assert cst_record is not None, "CST should exist after update_file_data"
        assert (
            cst_record["cst_code"] == initial_content
        ), "CST should match file content"
        ...
        assert "InitialClass" in [
            c["name"] for c in classes
        ], "InitialClass should exist"

        functions = test_db._fetchall(
            "SELECT name FROM functions WHERE file_id = ?", (file_id,)
        )
        assert len(functions) > 0, "Functions should exist"
        assert "initial_function" in [
            f["name"] for f in functions
        ], "initial_function should exist"
...
'''
        ...
        ), f"Second update should succeed: {result2.get('error')}"
        assert (
            result2.get("ast_updated") is True
        ), "AST should be updated after modification"
        assert (
            result2.get("cst_updated") is True
        ), "CST should be updated after modification"
        ...
        assert (
            updated_cst["cst_code"] == modified_content
        ), "CST should match modified file content"

        # Step 11: Verify entities were updated
        updated_classes = test_db._fetchall(
            "SELECT name FROM classes WHERE file_id = ?", (file_id,)
        )
        ...
        updated_functions = test_db._fetchall(
            "SELECT name FROM functions WHERE file_id = ?", (file_id,)
        )
        ...
        assert (
            "initial_function" not in function_names
        ), "initial_function should be removed"

        # Step 12: Verify chunks are deleted again (ready for re-chunking)
        chunks_after_modify = test_db._fetchall(
            "SELECT id FROM code_chunks WHERE file_id = ?", (file_id,)
        )
        assert (
            len(chunks_after_modify) == 0
        ), "Chunks should be deleted after modification, ready for re-chunking"
