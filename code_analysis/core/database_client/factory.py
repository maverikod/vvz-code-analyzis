"""
Factory for :class:`DatabaseClient` from server config (SQLite RPC vs PostgreSQL in-process).
...
def compute_socket_path_for_sqlite_driver(
    driver_config: Dict[str, Any], storage_db_path: Path
) -> str:
    """Same socket naming as :class:`~code_analysis.core.database_driver_manager.DatabaseDriverManager`."""
    driver_cfg = driver_config.get("config", {}) or {}
    db_path = driver_cfg.get("path")
    if db_path:
        db_name = Path(db_path).stem
    else:
        db_name = storage_db_path.stem
...
def create_database_client_from_config_path(
    ...
    *,
    ...
    """Build a :class:`DatabaseClient` for the configured driver type.

    - **postgres**: :class:`~code_analysis.core.database_driver_pkg.drivers.postgres.PostgreSQLDriver`
      + :class:`~code_analysis.core.database_driver_pkg.rpc_handlers.RPCHandlers` in-process
      (no Unix socket, no database driver subprocess).
    - **sqlite** / **sqlite_proxy**: Unix :class:`~code_analysis.core.database_client.rpc_client.RPCClient`
      to the driver subprocess; socket path matches ``DatabaseDriverManager``.
    """
    ...
    if driver_type not in ("sqlite", "sqlite_proxy"):
        logger.warning(
            "Unknown driver type %s; using Unix RPC socket transport like sqlite",
            driver_type,
        )
...
def create_worker_database_client(
    *,
    ...
    ``code_analysis.database.driver`` in ``config_path`` (PostgreSQL in-process or
    SQLite RPC subprocess). Do not construct :class:`DatabaseClient` with a raw
    ``socket_path`` in worker code.
    """
