...
Numbered s23 (not s11/s12 — both already taken by
``s11_read_throughput``/``s12_indexer_correctness`` at the time this suite
was added; renumbered from s22 to s23 during integration to resolve a
collision with ``s22_vector_dim_parity``) to avoid colliding with any
in-flight sibling suite.
