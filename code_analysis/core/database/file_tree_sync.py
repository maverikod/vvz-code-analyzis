...
def sync_file_to_db_atomic(
    ...
    *,
    ...
    def _sync() -> Dict[str, Any]:
        ...
        # separate step). Delegates to the single shared helper
        # (core/entity_cross_ref_builder.rebuild_entity_cross_ref_for_file) so
        # every write path that persists entities for a file rebuilds cross-ref
        # the SAME way - see that helper's docstring for the bug 3e7177d6
        # history (the CST-write/batch path used to skip this step entirely).
        #
        ...
        # execute_batch / commit round-trips). Calling the helper here with no
        # surrounding transaction means _in_transaction() is False, so
        # add_entity_cross_ref's (and delete_entity_cross_ref_for_file's) own
        # per-call commit guard commits each statement individually - correct
        # and safe standalone behavior (same as calling add_class/add_usage/etc.
        # outside a transaction), it just does not get the extra all-or-nothing
        # atomicity a wrapping transaction would add across the several
        # cross-ref edges of one file. A failure here must never fail the file
        # sync itself - the helper itself never raises.
        ...
        rebuild_entity_cross_ref_for_file(
            ...
            project_id,
            source_code,
            context=f"sync_file_to_db_atomic path={absolute_path}",
        )
