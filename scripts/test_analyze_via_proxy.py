#!/usr/bin/env python3
"""
Test analyze_project command via MCP Proxy.
...
async def test_analyze_project_via_proxy() -> bool:
    """Test analyze_project command via MCP Proxy."""
    logger.info("=" * 60)
    logger.info("TEST: analyze_project command via MCP Proxy")
    logger.info("=" * 60)
    logger.info(f"Proxy MCP URL: {PROXY_MCP_URL}")
    logger.info(f"Test project: {TEST_PROJECT_ROOT}")

    try:
        async with streamablehttp_client(PROXY_MCP_URL) as (
            read_stream,
            write_stream,
            _,
        ):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                logger.info("✅ Found analyze_project tool")
                ...
                logger.info("\n📋 Test 2: Calling analyze_project via proxy...")
                        ...
                        while waited < max_wait:
                            try:
                                ...
                                if isinstance(status_data, dict):
                                    ...
                                    logger.info(
                                        f"   Job status: {job_status} (waited {waited}s)"
                                    )

                                    if job_status == "completed":
                                        logger.info("✅ Job completed!")
                                        job_result = status_data.get("data", {}).get(
                                            "result", {}
                                        )
                                        ...
                                        error_info = status_data.get("data", {}).get(
                                            "error", "Unknown error"
                                        )
                                        ...
                                        logger.warning(
                                            f"⚠️  Unknown status: {job_status}"
                                        )
                                        await asyncio.sleep(wait_interval)
                                        waited += wait_interval
                                else:
                                    logger.warning(
                                        f"⚠️  Unexpected status format: {type(status_data)}"
                                    )
                                    await asyncio.sleep(wait_interval)
                                    waited += wait_interval
...
def main() -> None:
    """Main entry point."""
    try:
        result = anyio.run(test_analyze_project_via_proxy, backend="asyncio")
