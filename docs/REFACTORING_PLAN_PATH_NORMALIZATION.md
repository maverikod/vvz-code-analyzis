...
7. **⚠️ Найдены хардкоды в коде** (требуют замены на константы):
   - Числовые константы: `400` (max_lines), `10` (batch_size), `30` (poll_interval) в нескольких файлах
   - Строковые константы: игнорируемые директории `["__pycache__", "node_modules", ".git", "data", "logs"]` в нескольких файлах
   - Пути: `"data" / "code_analysis.db"`, `"./logs"` в main.py
   - Паттерны плейсхолдеров: хардкод в comprehensive_analyzer.py вместо `PLACEHOLDER_PATTERNS`
   - Подробности: см. раздел "Детальный анализ хардкодов в проекте"
...
3. ✅ Вынести все хардкоды из кода в константы - **✅ выполнено**:
   - ✅ `code_analysis/commands/code_mapper_mcp_command.py`: использует `DEFAULT_MAX_FILE_LINES`
   - ✅ `code_analysis/core/comprehensive_analyzer.py`: использует `DEFAULT_MAX_FILE_LINES`, `PLACEHOLDER_PATTERNS`
   - ✅ `code_analysis/commands/database_restore_mcp_commands.py`: использует `DEFAULT_IGNORE_PATTERNS`, `DATA_DIR_NAME`, `LOGS_DIR_NAME`
   - ✅ `code_analysis/commands/worker_management_mcp_commands.py`: использует константы
   - ✅ `code_analysis/main.py`: использует `DEFAULT_DB_PATH`, `LOGS_DIR_NAME`, `DEFAULT_BATCH_SIZE`, `DEFAULT_POLL_INTERVAL`
   - ✅ `code_analysis/commands/base_mcp_command.py`: использует `DEFAULT_MAX_FILE_LINES`
...
6. ✅ **🚨 КРИТИЧНО**: Обновить `config.py` для использования SettingsManager - **без этого замены хардкодов теряют смысл** - **✅ выполнено**
   - **Файл**: `code_analysis/core/config.py`
   - **Приоритет**: Критичный
   - **Статус**: ✅ Выполнено
   - **Выполненные действия**:
     - ✅ Интегрирован SettingsManager в ServerConfig для получения значений по умолчанию
     - ✅ Используется `get_settings()` для получения значений констант с учетом приоритета CLI > ENV > Constants
     - ✅ Обновлены все Field(default_factory=...) для использования SettingsManager через lambda функции
7. ✅ Обновить `main.py` для передачи CLI аргументов в SettingsManager
8. ✅ Обновить все модули для использования `get_settings()` вместо прямого доступа к константам - **✅ выполнено** (все хардкоды заменены на константы)
...
3. ⚠️ Заменить все вызовы `normalize_abs_path()` на новый метод - **частично выполнено**
4. ⚠️ Обновить все места использования - **частично выполнено**

---

### Этап 5.1: Полностью исключить старые методы нормализации 🚨 ПЕРВООЧЕРЕДНАЯ ЗАДАЧА

**Цель**: Полностью удалить `normalize_abs_path()` из кодовой базы и заменить на `normalize_file_path()` или `normalize_path_simple()`

**Файлы для изменения**:
- `code_analysis/core/database/files.py` - заменить все использования `normalize_abs_path()`
- `code_analysis/core/path_normalization.py` - убрать зависимость от `normalize_abs_path()`
- `code_analysis/core/project_resolution.py` - удалить функцию `normalize_abs_path()` после замены всех использований
- `tests/test_regression.py` - обновить тесты
- `tests/test_performance.py` - обновить тесты
- `tests/test_edge_cases.py` - обновить тесты
- `scripts/fix_database_paths.py` - обновить скрипт

**Стратегия замены**:
1. **Для методов, работающих с файлами и требующих валидации project_id**:
   - Использовать `normalize_file_path()` с валидацией
   - Примеры: `add_file()`, `update_file_data()`, `mark_file_deleted()`, `vectorize_file_immediately()`

2. **Для методов, работающих только с путями без валидации project_id**:
   - Использовать `normalize_path_simple()` (внутренняя функция, которая не требует project_id)
   - Примеры: `get_file_by_path()`, `get_file_id()`, `get_file_versions()`

3. **Для внутренних функций нормализации**:
   - Реализовать простую нормализацию пути внутри `path_normalization.py` без зависимости от `normalize_abs_path()`

**Действия**:
1. ✅ Заменить все использования `normalize_abs_path()` в `database/files.py` на `normalize_file_path()` или `normalize_path_simple()` - **выполнено**
2. ✅ Обновить `path_normalization.py` для реализации простой нормализации без зависимости от `normalize_abs_path()` - **выполнено** (реализована `_normalize_path_simple_internal()`)
3. ✅ Обновить все тесты для использования новых методов - **выполнено** (test_regression.py, test_edge_cases.py, test_performance.py)
4. ✅ Обновить скрипты для использования новых методов - **выполнено** (fix_database_paths.py)
5. ✅ Пометить `normalize_abs_path()` как deprecated в `project_resolution.py` - **выполнено** (функция оставлена для обратной совместимости с предупреждением)
6. ✅ Обновить импорты во всех файлах - **выполнено**
...
**Выполненные действия**:
1. ✅ Составлен полный список всех мест
2. ✅ Классифицированы по типам операций
3. ✅ Заменены на использование единых методов где необходимо
4. ✅ Добавлена валидация project_id в критических местах

**Использование старых методов (`normalize_abs_path`)**:
- ✅ **Допустимо** в методах, которые работают только с путями и не требуют валидации project_id:
  - `get_file_by_path()` - только нормализация пути для поиска в БД
  - `get_file_id()` - только нормализация пути для поиска в БД
  - `get_file_versions()` - только нормализация пути для поиска в БД
- ✅ **Заменено на `normalize_file_path()`** в методах, требующих валидации project_id:
  - `add_file()` - использует `normalize_file_path()` с валидацией project_id
  - `update_file_data()` - валидирует project_id через `load_project_id()`
  - `mark_file_deleted()` - использует `normalize_file_path()` с валидацией project_id
  - `vectorize_file_immediately()` - использует `normalize_file_path()` с валидацией project_id

**📝 Примечание**: Использование `normalize_abs_path()` в методах, которые не требуют валидации project_id, является допустимым и соответствует архитектуре проекта.
...
**🚨 КРИТИЧЕСКИЕ ПРИОРИТЕТЫ (выполнить немедленно)**:
1. **Этап 3.5.1** - Обновить `config.py` для использования SettingsManager (критично, без этого замены хардкодов теряют смысл)
2. **Этап 5.1** - Полностью исключить старые методы нормализации (`normalize_abs_path`) - первоочередная задача
...
5. **Этап 3.5.1** - Обновить `config.py` для использования SettingsManager (критично) ✅
6. **Этап 4** - Менеджер проектов с git (управление) ✅
7. **Этап 5** - Унификация нормализации (централизация) ✅
8. **Этап 5.1** - Полностью исключить старые методы нормализации (первоочередная задача) ✅
9. **Этап 6** - Валидация project_id (безопасность) ✅
10. **Этап 7** - Рефакторинг файлвочера (применение) ✅
11. **Этап 8** - Рефакторинг векторизатора (применение) ✅
12. **Этап 9** - Обновление всех мест (полнота) ✅
13. **Этап 10** - Тесты (проверка) ✅
14. **Этап 11** - Миграция (данные) ✅
15. **Этап 12** - Документация (финализация) ⚠️ (частично выполнено)
...
- [x] Все хардкоды вынесены в файл констант - **✅ выполнено** (все хардкоды заменены на константы, проверено)
- [x] Создан SettingsManager с приоритетом CLI > ENV > Constants
- [x] Все модули обновлены для использования констант - **✅ выполнено** (все хардкоды заменены на константы из constants.py, проверено)
- [x] 🚨 **КРИТИЧНО**: Обновить config.py для использования SettingsManager (без этого замены хардкодов теряют смысл, см. Этап 3.5.1) - **✅ выполнено**
...
- [x] 🚨 **ПЕРВООЧЕРЕДНАЯ ЗАДАЧА**: Полностью исключить старые методы нормализации (`normalize_abs_path`) - заменить на `normalize_file_path()` или `normalize_path_simple()` (см. Этап 5.1) - **✅ выполнено** (все использования заменены, функция помечена как deprecated)
...
**✅ Дополнительные выполненные пункты**:
- [x] Заменить хардкоды на константы в найденных файлах (см. Этап 3.5, пункт 3) - **✅ выполнено** (все хардкоды заменены, проверено)
- [x] Обновить использование старых методов нормализации (`normalize_abs_path`) - **✅ выполнено** (используется только где допустимо, критичные места используют `normalize_file_path` с валидацией)
...
**Числовые константы** (все хардкоды заменены на константы):
- `400` - максимальное количество строк в файле
  - ✅ `code_analysis/core/comprehensive_analyzer.py:39` - использует `DEFAULT_MAX_FILE_LINES`
  - ✅ `code_analysis/commands/code_mapper_mcp_command.py:575` - использует `DEFAULT_MAX_FILE_LINES`
  - ✅ `code_analysis/commands/base_mcp_command.py:275` - использует `DEFAULT_MAX_FILE_LINES`
  - ✅ Константа: `DEFAULT_MAX_FILE_LINES = 400`
- `30` - минимальная длина чанка, интервал опроса (секунды)
  - ✅ `code_analysis/main.py:889` - использует `DEFAULT_POLL_INTERVAL`
  - ✅ Константа: `DEFAULT_POLL_INTERVAL = 30`
- `60` - интервал сканирования (секунды), таймаут восстановления (секунды), таймаут команды (секунды)
  - ✅ Константа: `DEFAULT_SCAN_INTERVAL = 60`, `DEFAULT_COMMAND_TIMEOUT = 60.0`
- `10` - размер батча, задержка повтора (секунды), таймаут git команд (секунды)
  - ✅ `code_analysis/main.py:888` - использует `DEFAULT_BATCH_SIZE`
  - ✅ Константа: `DEFAULT_BATCH_SIZE = 10`, `DEFAULT_GIT_TIMEOUT = 10.0`
- `3` - количество попыток повтора, максимальное количество пустых итераций
  - ✅ Константа: `DEFAULT_RETRY_ATTEMPTS = 3`, `DEFAULT_MAX_EMPTY_ITERATIONS = 3`
- `5` - порог отказов, количество резервных копий логов, задержка (секунды), максимальное количество попыток подключения к БД
  - ✅ Константа: `DEFAULT_FAILURE_THRESHOLD = 5`, `DEFAULT_LOG_BACKUP_COUNT = 5`, `DB_CONNECTION_MAX_RETRIES = 5`
- `2` - порог успешных запросов, множитель задержки, задержка между попытками подключения к БД (секунды)
  - ✅ Константа: `DEFAULT_SUCCESS_THRESHOLD = 2`, `DEFAULT_BACKOFF_MULTIPLIER = 2.0`, `DB_CONNECTION_RETRY_DELAY = 2.0`
- `300` - максимальная задержка (секунды), максимальная длительность сканирования (секунды)
  - ✅ Константа: `DEFAULT_MAX_BACKOFF = 300.0`, `DEFAULT_MAX_SCAN_DURATION = 300`
- `10485760` - максимальный размер лог-файла (10 MB)
  - ✅ Константа: `DEFAULT_LOG_MAX_BYTES = 10485760`
- `384` - размерность вектора
  - ✅ Константа: `DEFAULT_VECTOR_DIM = 384`
- `15000` - порт сервера по умолчанию
  - ✅ Константа: `DEFAULT_SERVER_PORT = 15000`
- `8009` - порт чанкера по умолчанию
  - ✅ Константа: `DEFAULT_CHUNKER_PORT = 8009`
- `8001` - порт сервиса эмбеддингов по умолчанию
  - ✅ Константа: `DEFAULT_EMBEDDING_PORT = 8001`
- `0.1` - толерантность для сравнения времени модификации файлов (секунды)
  - ✅ Константа: `FILE_MODIFICATION_TOLERANCE = 0.1`
- `0.001` - эпсилон для обновления last_modified
  - ✅ Константа: `LAST_MODIFIED_EPSILON = 0.001`
- `1.0` - интервал опроса для proxy драйвера (секунды)
  - ✅ Константа: `PROXY_DRIVER_POLL_INTERVAL = 1.0`

**Пути и имена файлов** (все хардкоды заменены на константы):
- Все пути к директориям и файлам вынесены в константы
  - ✅ `code_analysis/main.py:332,455` - использует `DEFAULT_DB_PATH`
  - ✅ `code_analysis/main.py:340,462` - использует `LOGS_DIR_NAME`
  - ✅ Константы: `DEFAULT_DB_PATH = "data/code_analysis.db"`, `LOGS_DIR_NAME = "logs"`, `DATA_DIR_NAME = "data"`
- Все расширения файлов должны быть в константах
  - ✅ Константа: `CODE_FILE_EXTENSIONS`, `CONFIG_FILE_EXTENSIONS`
- Все имена файлов (projectid, .gitignore) должны быть в константах
  - ✅ Константы: `PROJECTID_FILENAME = "projectid"`, `GITIGNORE_FILENAME = ".gitignore"`

**Места использования хардкодов** (проверено, все исправлено):

**✅ Все хардкоды заменены на константы**:
- `code_analysis/core/file_watcher_pkg/scanner.py`: использует `CODE_FILE_EXTENSIONS`, `DEFAULT_IGNORE_PATTERNS`
- `code_analysis/core/config.py`: использует константы из `constants.py`
- `code_analysis/core/comprehensive_analyzer.py`: 
  - ✅ строка 39: использует `DEFAULT_MAX_FILE_LINES`
  - ✅ строка 55: использует `PLACEHOLDER_PATTERNS`
- `code_analysis/commands/code_mapper_mcp_command.py`:
  - ✅ строка 575: использует `DEFAULT_MAX_FILE_LINES`
  - ✅ строка 648: это сообщение об ошибке, не хардкод паттернов
- `code_analysis/commands/database_restore_mcp_commands.py`:
  - ✅ строка 81: использует `DEFAULT_IGNORE_PATTERNS`, `DATA_DIR_NAME`, `LOGS_DIR_NAME`
- `code_analysis/commands/worker_management_mcp_commands.py`:
  - ✅ строка 246: использует `DEFAULT_IGNORE_PATTERNS`, `DATA_DIR_NAME`, `LOGS_DIR_NAME`
- `code_analysis/main.py`:
  - ✅ строки 332, 455: используют `DEFAULT_DB_PATH`
  - ✅ строки 340, 462: используют `LOGS_DIR_NAME`
  - ✅ строки 888-889: используют `DEFAULT_BATCH_SIZE`, `DEFAULT_POLL_INTERVAL`
- `code_analysis/commands/base_mcp_command.py`:
  - ✅ строка 275: использует `DEFAULT_MAX_FILE_LINES`

**📝 Примечание**: Все найденные хардкоды успешно заменены на константы из `constants.py`. Дополнительных хардкодов не обнаружено.

---

## Анализ использования старых методов нормализации

**Статус**: ✅ Проверено и обновлено

### Использование `normalize_abs_path()` vs `normalize_file_path()`

**✅ Допустимое использование `normalize_abs_path()`** (методы, которые работают только с путями и не требуют валидации project_id):
- `code_analysis/core/database/files.py`:
  - `get_file_by_path()` - только нормализация пути для поиска в БД
  - `get_file_id()` - только нормализация пути для поиска в БД
  - `get_file_versions()` - только нормализация пути для поиска в БД

**✅ Заменено на `normalize_file_path()`** (методы, требующие валидации project_id):
- `code_analysis/core/database/files.py`:
  - `add_file()` - использует `normalize_file_path()` с валидацией project_id
  - `update_file_data()` - валидирует project_id через `load_project_id()`
  - `mark_file_deleted()` - использует `normalize_file_path()` с валидацией project_id
...
**📝 Примечание**: Использование `normalize_abs_path()` в методах, которые не требуют валидации project_id, является допустимым и соответствует архитектуре проекта. Критичные методы, которые работают с файлами и требуют валидации project_id, используют `normalize_file_path()`.
