...
def load_file_to_tree(
    ...
    _build_tree_index(tree, node_types=node_types, max_depth=max_depth, include_children=include_children)
...
def _build_tree_index(
    ...
    def visit(node: cst.CSTNode, depth: int) -> None:
        ...
        # Generate node_id
        node_id = _generate_node_id(node, start_line, start_col, end_line, end_col, class_stack, func_stack)

        # Store node
        tree.node_map[node_id] = node
        if parent:
            parent_id = _get_node_id_for_node(parent, positions, class_stack, func_stack)
        ...
        if include_children:
            for child in node.children:
                ...
                if child_pos:
                    try:
                        child_start_line = (
                            child_pos.start.line if hasattr(child_pos, "start") and hasattr(child_pos.start, "line") else 1
                        )
                        child_start_col = (
                        ...
                        child_end_line = child_pos.end.line if hasattr(child_pos, "end") and hasattr(child_pos.end, "line") else 1
                        child_end_col = (
                            child_pos.end.column if hasattr(child_pos, "end") and hasattr(child_pos.end, "column") else 0
                        )
                        child_id = _generate_node_id(child, child_start_line, child_start_col, child_end_line, child_end_col, class_stack, func_stack)
...
def _generate_node_id(
    ...
    return f"{kind}:{qualname}:{node_type}:{start_line}:{start_col}-{end_line}:{end_col}"
...
def _get_node_id_for_node(
    ...
    try:
        ...
        end_line = pos.end.line if hasattr(pos, "end") and hasattr(pos.end, "line") else 1
        end_col = pos.end.column if hasattr(pos, "end") and hasattr(pos.end, "column") else 0
        return _generate_node_id(node, start_line, start_col, end_line, end_col, class_stack, func_stack)
    except (AttributeError, TypeError):
        return None
...
def _get_node_qualname(node: cst.CSTNode, class_stack: List[str], func_stack: List[str]) -> Optional[str]:
