...
Usage:
    ...
    def __init__(self):
        self.vectorstorecrud = VectorStoreCRUD()
    def upsert_chunk(self, chunk):
        return self.vectorstorecrud.upsert_chunk(chunk)
    def upsert_chunks(self, chunks):
        return self.vectorstorecrud.upsert_chunks(chunks)
    def get_chunk(self, uuid, include_vectors):
        return self.vectorstorecrud.get_chunk(uuid, include_vectors)
    def get_chunks(self, uuids, include_vectors):
        return self.vectorstorecrud.get_chunks(uuids, include_vectors)
    def delete_chunk(self, uuid):
        return self.vectorstorecrud.delete_chunk(uuid)
    def delete_chunks(self, uuids):
        return self.vectorstorecrud.delete_chunks(uuids)
    def delete_records(self, metadata_filter):
        return self.vectorstorecrud.delete_records(metadata_filter)
    def hard_delete_records(self, uuids):
        return self.vectorstorecrud.hard_delete_records(uuids)
    def force_delete_by_uuids(self, uuids, force):
        return self.vectorstorecrud.force_delete_by_uuids(uuids, force)
    def chunk_deferred_cleanup(self, dry_run, batch_size):
        return self.vectorstorecrud.chunk_deferred_cleanup(dry_run, batch_size)
    def chunk_hard_delete(self, uuids, confirm):
        return self.vectorstorecrud.chunk_hard_delete(uuids, confirm)
    def get_all_record_ids(self, uuids):
        return self.vectorstorecrud.get_all_record_ids(uuids)
    def count_records(self):
        return self.vectorstorecrud.count_records()
    def save_metadata(self, uuid, metadata, idx):
        return self.vectorstorecrud.save_metadata(uuid, metadata, idx)
    def get_metadata(self, uuid):
        return self.vectorstorecrud.get_metadata(uuid)
    def delete_record(self, uuid):
        return self.vectorstorecrud.delete_record(uuid)
    def search(self, chunk_query, limit, offset, include_vectors, include_deleted):
        return self.vectorstoresearch.search(chunk_query, limit, offset, include_vectors, include_deleted)
    def hybrid_search(self, chunk_query):
        return self.vectorstoresearch.hybrid_search(chunk_query)
    def search_bm25(self, query, limit, k1, b):
        return self.vectorstoresearch.search_bm25(query, limit, k1, b)
    def get_search_stats(self):
        return self.vectorstoresearch.get_search_stats()
    def get_id_to_idx_mapping(self):
        return self.vectorstoresearch.get_id_to_idx_mapping()
    def _should_include_vectors(self, metadata_filter):
        return self.vectorstoresearch._should_include_vectors(metadata_filter)
    def _combine_hybrid_results(self, semantic_results, bm25_results, chunk_query):
        return self.vectorstoresearch._combine_hybrid_results(semantic_results, bm25_results, chunk_query)
    def verify_data_consistency(self):
        return self.vectorstoremaintenance.verify_data_consistency()
    def auto_verify_after_operation(self, operation_name):
        return self.vectorstoremaintenance.auto_verify_after_operation(operation_name)
    def flush_all(self):
        return self.vectorstoremaintenance.flush_all()
    def deferred_cleanup(self, background, batch_size):
        return self.vectorstoremaintenance.deferred_cleanup(background, batch_size)
    def find_duplicates(self, metadata_filter, batch_size, background):
        return self.vectorstoremaintenance.find_duplicates(metadata_filter, batch_size, background)
    def find_duplicate_uuids(self, metadata_filter):
        return self.vectorstoremaintenance.find_duplicate_uuids(metadata_filter)
    def _find_duplicates_in_uuids(self, uuids):
        return self.vectorstoremaintenance._find_duplicates_in_uuids(uuids)
    def _scan_uuids(self):
        return self.vectorstoremaintenance._scan_uuids()
    def reindex_all(self, embedding_service, lock_database, background):
        return self.vectorstoremaintenance.reindex_all(embedding_service, lock_database, background)
    def full_reindex(self, embedding_service):
        return self.vectorstoremaintenance.full_reindex(embedding_service)
    def cleanup_orphans(self, background):
        return self.vectorstoremaintenance.cleanup_orphans(background)
    def get_maintenance_statistics(self):
        return self.vectorstoremaintenance.get_maintenance_statistics()
    def get_operation_status(self, operation_id):
        return self.vectorstoremaintenance.get_operation_status(operation_id)
    def is_deleted(self, uuids):
        return self.vectorstoremaintenance.is_deleted(uuids)
    def get_all_uuids(self, include_deleted):
        return self.vectorstoremaintenance.get_all_uuids(include_deleted)
    def get_deleted_uuids(self):
        return self.vectorstoremaintenance.get_deleted_uuids()
    def get_index_info(self):
        return self.vectorstoreinfo.get_index_info()
    def info(self):
        return self.vectorstoreinfo.info()
    def _get_source_id_statistics(self, uuids):
        return self.vectorstoreinfo._get_source_id_statistics(uuids)
    def _get_redis_statistics(self):
        return self.vectorstoreinfo._get_redis_statistics()
    def _get_metadata_statistics(self):
        return self.vectorstoreinfo._get_metadata_statistics()
    def _get_cache_metrics(self):
        return self.vectorstoreinfo._get_cache_metrics()
    def _get_performance_metrics(self):
        return self.vectorstoreinfo._get_performance_metrics()
    def _get_complexity_metrics(self):
        return self.vectorstoreinfo._get_complexity_metrics()
    def _get_error_metrics(self):
        return self.vectorstoreinfo._get_error_metrics()
    def _get_resource_metrics(self):
        return self.vectorstoreinfo._get_resource_metrics()
    def _get_business_metrics(self):
        return self.vectorstoreinfo._get_business_metrics()
    def vector_size(self):
        return self.vectorstoreutils.vector_size()
    def ping(self):
        return self.vectorstoreutils.ping()
    def is_valid_uuid4(self, uuid_str):
        return self.vectorstoreutils.is_valid_uuid4(uuid_str)
    def _format_vector_log(self, vector, max_show):
        return self.vectorstoreutils._format_vector_log(vector, max_show)
    def _restore_vectors_from_redis(self, chunks_data):
        return self.vectorstoreutils._restore_vectors_from_redis(chunks_data)
    def index_chunk_for_bm25(self, chunk):
        return self.vectorstoreutils.index_chunk_for_bm25(chunk)
    def initialize_bm25(self):
        return self.vectorstoreutils.initialize_bm25()
...
Usage:
    ...
    async def full_reindex(self, embedding_service) -> None:
        ...
        try:
            ...
            # 3. Векторизовать тексты и добавить в FAISS
            if embedding_service and self.faiss_service:
                ...
                if vectors:
                    ...
                    try:
                        ...
                        # Обновить маппинги FAISS в Redis
