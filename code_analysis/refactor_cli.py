...
def split_class(file: Path, config: Path, dry_run: bool) -> None:
    """
    ...
    try:
        ...
        if dry_run:
            ...
            is_valid, errors = splitter.validate_split_config(src_class, split_config)
            if is_valid:
...
def extract_superclass(file: Path, config: Path, dry_run: bool) -> None:
    """
    ...
    try:
        ...
        if dry_run:
            ...
            is_safe, conflict_error = extractor.check_multiple_inheritance_conflicts(
...
def merge_classes(file: Path, config: Path, dry_run: bool) -> None:
    """
