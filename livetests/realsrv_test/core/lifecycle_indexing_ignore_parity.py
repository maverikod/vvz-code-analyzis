...
async def run_update_indexes_excludes_watcher_ignored_test_data(
    ...
    created_projects: List[Tuple[str, str]] = []
    try:
        ...
        # Routed through the shared purge helper (bug d5835fbf): the old
        # bare project_set_mark_del call here left trash-only residue (no
        # permanently_delete_from_trash follow-up) for both the source and
        # target throwaway projects every run (13 leaked
        # "verify_ignore_parity_*" trash entries observed).
        for leftover_project_id, leftover_project_name in created_projects:
            await purge_disposable_project(
                client, leftover_project_id, leftover_project_name
            )
...
async def _seed_source_project(
    ...
    *,
    ...
    Args:
        ...
        project_name: Name to create the source project under -- generated by
            the caller (not here) so it stays known to the caller for
            teardown even when this function fails before returning it.
...
async def _run_ignore_parity_check(
    ...
    *,
    ...
    Args:
        ...
        session_id: Open file-session id (not project-scoped).
        created_projects: Appended with every ``(project_id, project_name)``
            this check creates (source and target), so the caller can
            best-effort clean up all of them regardless of where a failure
            happens.
    ...
    source_project_name = f"verify_ignore_parity_src_{uuid.uuid4().hex[:8]}"
    ...
    if source_project_id:
        created_projects.append((source_project_id, source_project_name))
    if seed_error:
        return _outcome(Status.FAILED, truncate(seed_error))

    target_project_name = f"verify_ignore_parity_{uuid.uuid4().hex[:8]}"
    ...
    created_projects.append((target_project_id, target_project_name))
