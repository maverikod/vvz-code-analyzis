...
def _write_replace_result_atomically(
    ...
    database = None
    abs_resolved = str(target.resolve())
    try:
...
def run_replace_flow(
    ...
    # If source did not actually change, reset replaced counter.
    # apply_replace_ops counts ops dispatched, not actual textual changes.
    actual_replaced = stats.get("replaced", 0) if diff else 0
